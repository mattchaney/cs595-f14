...
def loadMovieLens():
	...
	for line in open('u.item'):
		mid, title, theater, video, url = line.split('|')[:5]
		genre = [int(item.strip()) for item in line.split('|')[5:]]
		movies[mid]= {'title': title, 'url': url, 
					'genre':genre, 'theater': theater, 'video': video}
...
def get_avg(mid, user_filter=lambda x: True):
	ratings = []
	for user, user_prefs in prefs.iteritems():
		if user_filter(users[user]) and user_prefs.has_key(movies[mid]['title']):
			ratings.append(user_prefs[movies[mid]['title']])
	return numpy.mean(ratings)

def f(fil=lambda x: True):
	for user, user_prefs in prefs.iteritems():
		if fil(users[user]):
			print users[user]

def get_top_rated(sorted_avg):
	top = sorted_avg[0][1]
	top_titles = []
	i = 0
	while i < 5 or sorted_avg[i][1] == top:
		top_titles.append(sorted_avg[i])
		if i < 5 and sorted_avg[i][1] != top:
			top = sorted_avg[i][1]
		i += 1
	return top_titles

def count_ratings(mid):
	num = 0
	for user, user_prefs in prefs.iteritems():
		if user_prefs.has_key(movies[mid]['title']):
			num += 1
	return num

def get_top_count():
	top = sorted_counts[0][1]
	top_counts = []
	i = 0
	while i < 5 or sorted_counts[i][1] == top:
		top_counts.append(sorted_counts[i])
		if i < 5 and sorted_counts[i][1] != top:
			top = sorted_counts[i][1]
		i += 1
	return top_counts
...
# print "Working #1"
# averages_all = {movie['title']: get_avg(mid) for mid, movie in movies.iteritems()}
# sorted_avg_all = sorted(averages_all.items(), key=operator.itemgetter(1), reverse=True)
# top_rated = get_top_rated(sorted_avg_all)
# print top_rated

# 2.  What 5 movies received the most ratings? Show the movies and the number of ratings sorted by number of ratings.
print "Working #2"
counts = {movie['title']: count_ratings(mid) for mid, movie in movies.iteritems()}
sorted_counts = sorted(counts.items(), key=operator.itemgetter(1), reverse=True)
top_counts = get_top_count()
print top_counts

# 3.  What 5 movies were rated the highest on average by women? Show the movies and their ratings sorted by ratings.
print "Working #3"
averages_w = {movie['title']: get_avg(mid, user_filter=lambda x: x['gender']=='F') for mid, movie in movies.iteritems()}
sorted_avg_w = sorted(averages_w.items(), key=operator.itemgetter(1), reverse=True)
top_rated_w = get_top_rated(sorted_avg_w)
print top_rated_w
